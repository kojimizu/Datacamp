...
- Stationary process have distributional stability over time

Observed time series
- Fluctuate randomly
- But behave similar from one time period to the next

__Weak stationarity - I:__ 
mean, variance, covariance constant over time.

E.g., $Y_1, Y_2,...$ is weakly stationary process if: 

- mean $\mu$ of $Y_t$ is same (constant) for all $t$
- Variance $\sigma^2$ of $Y_t$ is same (constant) for all $t$
- And ...

__Weak stationarity - II__
- Covariance of $Y_t$ and $Y_s$ is same (constant) for alll $|t-s|=h$, for all $h$. For example, if the process is weakl stationary, 

$$
Cov(Y_2, Y_5) = Cov(Y_7, Y_10) 
$$

since each pair is separated by three units of time.

A stationary process can be modeled with fewer parameters. For example, we do not need a different expectation for each $Y_t$; rather they all have a common expectation, $mu$.
...
- The __changes__ in the series are often approximately stationary
- A stationary series should show random iscillation around some fixed level; a phenomenon called __mean-reversion__
...
wn_drift <- arima.sim(model=list(c(0,0,0)), mean=0.4, n=100)
...
When we want to check the log returns
```{r eval=FALSE}
...
```{r}
EuStockMarkets_tibble <- 
  EuStockMarkets %>% 
  as.tibble() %>% 
  mutate(time = 1:as.integer(count(as.tibble(EuStockMarkets))))
...
For checking the covariance, we use the `cov(var1, var2)` function. 

## Correlations
- standardized version of covariance
- +1: perfectly positive linear relationship
- -1: perfectily negative linear relationship
...
# correlation of stock A "today" and stock A "yersterday"
...
# autocorrelation of stock_A by lag
acf(stock_A, lag.max = 2, plot=FALSE)

# autocorrelation - plot=TRUE
acf(stock_A, plot=TRUE)
# estimated ACF in y-axis, lag on the x-axis
```
...
$$
, where $\epsilon_t$ is mean zero white noise (WN), and there are three parameters.

1. Mean $\mu$
2. Slope $\phi$
3. white noise variance $\sigma^2$

1) If slope $\phi=0$, then $Y_t = \mu + \epsilon_t$
$Y_t$ is white noise $(\mu, \sigma_\epsilon^2)$

2) If slope $\phi \neq 0$, $Y_t$ depends on both $epsilon_t$ and $Y_{t-1}$ and the process {$Y_t$} is autocorrelated.

Large values of $\phi$ lead to greater autocorrelation. Negative values of $\phi$ result in oscillatory time series 

## Simulate the autoregression (AR) model
...
## Poersitence and anti-persistence
...
(Today - Mean) = Slope * (Yesterday - Mean) + Noise \\
Y_t - \mu = \phi(Y_{t-1} - \mu) + \epsilon_t \\
\epsilon_t WhiteNoise(0, \hat{\sigma_\epsilon^2})
$$
```{r}
...
$ar1 = \phi$, intercept = $\mu$, sigma^2 =$\sigma^2_\epsilon$

### AR processes: Fitted values

- AR fitted values
$Today = Mean + Slope * (Yersterday - Mean)$
$$
...
## Estimate the autoregression (AR) model
...
(Today - Mean) = Slope*(Yesterday - Mean) + Noise \\
Y_t - \mu = \Phi(Y_{t-1}-\mu) + \epsilon_t \\
where \epsilon_t \sim WhiteNoise(0, \sigma_{\epsilon}^2) \neq
$$

- Autocorreations: 
- Fitted values: quite similar.
